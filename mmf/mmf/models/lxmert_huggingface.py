...
# )
# @registry.register_model("lxmert_transformers")
# class LXMERTQuestionAnswering(BaseModel):
#     def __init__(self, config):
...
#     @classmethod
#     def config_path(cls):
#         return "configs/models/lxmert_transformers/pretrain.yaml"
#
#     def build(self):
...
#             ' '.join(sample_list["text"][0]),
#             add_special_tokens=False,
#             return_tensors="pt"
#         )
#         visual_feats = sample_list['image_feature_0']
#         bboxes = sample_list['image_info_0']['bbox'][0]
#         # normalization of bboxes between 0 and 1
#         visual_pos = (bboxes - bboxes.min()) / (bboxes.max() - bboxes.min())
#         visual_pos = torch.from_numpy(visual_pos).unsqueeze(0).to(visual_feats.device)
#         output = self.model(input_ids=inputs.input_ids.to(visual_feats.device),
#                     visual_feats=visual_feats,
#                     visual_pos=visual_pos,
#                     return_dict=True)
#         output_dict = {}
#         output_dict["logits"] = output.question_answering_score
#         output_dict["masked_lm_loss"] = 0
#         output_dict["loss"] = 0
#         print("done")
#         return output_dict
#
